...
public class PlayerController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (titila)
        {
            tiempoTitila += Time.deltaTime;
            if (tiempoTitila >= 0.01)
            {
                maloMalo.SetActive(false);
                if (tiempoTitila >= 0.1)
                {
                    maloMalo.SetActive(true);
                    if (tiempoTitila >= 0.15)
                    {
                        maloMalo.SetActive(false);
                        if (tiempoTitila >= 0.2)
                        {
                            maloMalo.SetActive(true);
                            if (tiempoTitila >= 0.25)
                            {
                                maloMalo.SetActive(false);
                                if (tiempoTitila >= 0.30)
                                {
                                    maloMalo.SetActive(true);
                                    tiempoTitila = 0;
                                    titila = false;
